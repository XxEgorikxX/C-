...
tuple<vector<Coord>, double, vector<string>> dijkstra(const Graph& graph, const Coord& start, const Coord& end) {
    ...
    std::priority_queue<QueueItem, vector<QueueItem>, decltype(cmp)> pq(cmp);
    ...
    while (!pq.empty()) {
        ...
        if (std::find(seen.begin(), seen.end(), cur_node) != seen.end()) {
            continue;
        }

        seen.push_back(cur_node);

        // Получаем список соседей, если он существует
        auto graph_it = graph.find(cur_node);
        if (graph_it != graph.end()) {
            for (auto& nb_pair : graph_it->second) {
                Coord nb = nb_pair.first;
                double weight = nb_pair.second;
                double new_dist = cur_dist + weight; 

                if (shortest_paths.find(nb) == shortest_paths.end() ||
                    new_dist < shortest_paths[nb].second) {
                    shortest_paths[nb] = make_pair(cur_node, new_dist);
                    pq.push(make_pair(new_dist, nb));
                }
            }
        }
    }
    ...
    if (shortest_paths.find(end) == shortest_paths.end()) {
        return make_tuple(route, std::numeric_limits<double>::infinity(), street_names);
    }
    ...
    while (true) {
        route.push_back(cur);
        auto ancestor = shortest_paths[cur].first;
    ...
    double total_dist = shortest_paths[end].second;
    return make_tuple(route, total_dist, street_names);
}
...
int main() {
    ...
    map<string, Coord> nodes;
    vector<EdgeItem> edges;
    std::tie(nodes, edges) = read_graphml("berlin.graphml");
    Coord start, end;
    string name;
    std::tie(start, end, name) = edges[edges.size() - 1];
    string start_street_query = "Frischborner Weg";
    string end_street_query = "Gadebuscher Weg";
